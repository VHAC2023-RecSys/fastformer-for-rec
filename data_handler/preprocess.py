...
class NewsInfo:
    def __init__(self, args, mode="train", category_dict=None, subcategory_dict=None):
        ...
        if self.mode != "train":
            self.category_dict, self.subcategory_dict = category_dict, subcategory_dict
        else:
    ...
    def sent_tokenize(self, sent, max_len):
        ...
        sent_split = self.tokenizers(
            sent, max_length=max_len, pad_to_max_length=True, truncation=True
        )
        return sent_split
    ...
    def _parse_news_attrs(self, attr_raw_values):
        parser = {
            ...
            "abstract": (
                self.sent_tokenize,
                [],
                {"max_len": self.args.num_words_abstract},
            ),
        ...
        news_attrs = [
            self._parse_news_attr(attr_name, parser[attr_name], attr_raw_value)
            for attr_name, attr_raw_value in zip(
    ...
    def parse_line(self, line: str):
        ...
        self.update_dict(
            self.news, doc_id, [title, abstract, body, category, subcategory]
        )
...
def get_doc_input(news, news_index, category_dict, subcategory_dict, args):
    ...
    if "abstract" in args.news_attributes:
        ...
        news_abstract_attmask = np.zeros(
            (news_num, args.num_words_abstract), dtype="int32"
        )
    else:
        news_abstract = None
...
def infer_news(model, device, news_combined, batch_size=64):
    ...
    news_dataloader = DataLoader(news_dataset, batch_size=batch_size, num_workers=0)
