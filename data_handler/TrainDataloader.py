...
class DataLoaderTrainForSpeedyRec(IterableDataset):
    ...
    def dynamic_batch(self):
        ...
        for batch in self.sampler_batch:
            ...
            (
                ...
                end_inx,
                batch,
            ) = self.gen_batch_for_two_stage(
                encode_set, cache_set, uid_click_docs, uid_sample_news
            )
        ...
        if use_cache:
            ...
            for n in news_set:
                if n == 0:
                    continue
                if (
                    self.global_step - self.cache_state[n]
                    <= self.args.max_step_in_cache
                ):
                    cache_set.append(n)
                else:
                    encode_set.append(n)
    ...
    def gen_batch_for_two_stage(
        self, encode_news_set, cache_news_set, uid_click_docs, uid_sample_news
    ):
        """
        ...
        if self.add_pad_news:
            add_pad_num = math.ceil(
                len(encode_news_set) / self.world_size
            ) * self.world_size - len(encode_news_set)
        ...
        max_hist_len = (
            max([len(x) for x in uid_click_docs]) + 1
        )  # add a learnable pad_doc
    ...
    def _process(self, batch):
        ...
        for line in batch:
            ...
            if len(sess_neg) > 0:
                neg_index = self.news_sample(
                    list(range(len(sess_neg))), self.args.npratio
                )
