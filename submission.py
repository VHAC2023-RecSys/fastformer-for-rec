...
def prediction(model, args, device, category_dict, subcategory_dict):
    ...
    with torch.no_grad():
        ...
        dataloader = DataLoaderLeader(
            ...
            data_dirs=os.path.join(args.root_data_dir, f"test/"),
        ...
        for cnt, (impids, log_vecs, log_mask, candidate_vec) in enumerate(
            dataloader.generate_batch()
        ):
            if args.enable_gpu:
            ...
            for id, user_vec, news_vec in zip(impids, user_vecs, candidate_vec):
