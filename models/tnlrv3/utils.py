...
class Seq2seqDatasetForTuringNLRv3(torch.utils.data.Dataset):
    def __init__(
        ...
        finetuning_method="v1",
        target_mask_prob=-1.0,
        num_max_mask_token=0,
    ...
    def __getitem__(self, _idx):
        ...
        source_ids = self.__trunk(
            [self.cls_id] + feature.source_ids,
            self.max_source_len,
        ...
        if self.finetuning_method == "v0":
            ...
            num_masked_token = min(
                self.num_max_mask_token, int(self.target_mask_prob * num_target_tokens)
            )
...
def get_max_epoch_model(output_dir):
    ...
    if (not fn_model_list) or (not fn_optim_list):
        return None
    both_set = set(
        [int(os.path.dirname(fn).split("-")[-1]) for fn in fn_model_list]
...
def report_length(length_counter, total_count):
    ...
    while a < max_len:
        ...
        if cc > 0:
            logger.info(
                "%d ~ %d = %d, %.2f%%" % (a, a + 16, cc, (tc * 100.0) / total_count)
            )
        a += 16


def serialize_str(x):
    return "{}".format(x).encode("ascii")
...
def load_and_cache_examples(
    ...
    if cached_features_file is not None and os.path.isfile(cached_features_file):
        ...
        cached_features_file is not None
        and os.path.isdir(cached_features_file)
        ...
        if local_rank in [-1, 0] and cached_features_file is not None:
            if lmdb_cache:
                ...
                logger.info(
                    "Saving features into cached lmdb dir %s", cached_features_file
                )
            else:
