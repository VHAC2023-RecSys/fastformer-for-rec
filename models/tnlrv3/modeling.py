...
from .convert_state_dict import (
        ...
        **kwargs,
        ...
        if model_type is not None and "state_dict" not in kwargs:
            if model_type in cls.supported_convert_pretrained_model_archive_map:
                pretrained_model_archive_map = (
                    cls.supported_convert_pretrained_model_archive_map[model_type]
                )
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if split_lengths:
            ...
            outputs = []
            sum_length = 0
            for query, _key, _value, part_length in zip(
                query_parts, key_parts, value_parts, split_lengths
            ):
...
class BertLayer(nn.Module):
    ...
    def forward(
        self, hidden_states, attention_mask=None, split_lengths=None, rel_pos=None
    ):
        self_attention_outputs = self.attention(
            hidden_states, attention_mask, split_lengths=split_lengths, rel_pos=rel_pos
        )
...
class LabelSmoothingLoss(_Loss):
    ...
    def forward(self, output, target):
        ...
        return (
            F.kl_div(output, model_prob, reduction="none")
            .view(batch_size, num_pos, -1)
            .sum(2)
        )
...
class TuringNLRv3ForSequenceToSequenceWithPseudoMask(TuringNLRv3ForSequenceToSequence):
    ...
    def create_attention_mask(
        ...
        weight = torch.cat(
            (torch.zeros_like(source_position_ids), target_span_ids, -target_span_ids),
            dim=1,
        )
        ...
        true_tokens = (0 <= to_weight) & (
            torch.cat((source_mask, target_mask, target_mask), dim=1) == 1
        ).unsqueeze(1)
    ...
    def forward(
        ...
        source_mask, source_position_ids = create_mask_and_position_ids(
            num_source_tokens, source_len
        )
        target_mask, target_position_ids = create_mask_and_position_ids(
            num_target_tokens,
            target_len,
            offset=None if target_no_offset else num_source_tokens,
        )
        ...
        pseudo_sequence_output = sequence_output[
            :,
...
class TuringNLRv3ForSequenceToSequenceUniLMV1(TuringNLRv3ForSequenceToSequence):
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        target_sequence_output = sequence_output[
            :,
            source_len:,
        ]
        ...
        pseudo_lm_loss = loss_mask_and_normalize(masked_lm_loss.float(), masked_weight)
